...
The bash tool has the `exec` approval tier. `bash.patterns` rules can explicitly `allow`, `deny`, or `prompt`. By default, allow rules must match the entire command and cannot approve shell-control syntax. The opt-in `bash.allowCompoundCommands: true` additionally recognizes only flat chains of two or more literal commands separated by unquoted `&&`. It resolves the ordered rules independently against each original raw segment. The chain is allowed at the `write` tier only when every segment explicitly resolves to `allow`; any resolved deny wins, otherwise a resolved prompt or unmatched segment prompts. A catchall prompt therefore does not override an earlier per-segment allow, while a broad tool-level allow does not authorize an unmatched segment.

A `deny` or `prompt` pattern matching the full chain but no individual segment remains a whole-chain restriction. Other rules use per-segment first-match ordering; later broad restrictions do not override earlier segment allows. Explicit restrictions resolve before critical checks can produce a mode-dependent override.

Literal quoted arguments are accepted, but expansions, assignments, other control operators, redirections, globbing, newlines, malformed syntax, and shell-state-changing builtins (`cd`, `source`, `eval`, and similar) do not qualify. These inputs retain legacy approval behavior. Critical destructive and remote-fetch-and-execute checks still inspect the whole input and its segments, so an allowed prefix cannot conceal a critical later segment. Approval does not rewrite execution: the shell receives the original command, preserving native `&&` short-circuiting.
